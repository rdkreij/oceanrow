...
from plt_utils import plt_utils

class parameter_sweeper:
    '''
    Helper class to sweep various parameter values for an ocean row.
    '''
    def __init__(self, default_parameters, run_parameters):
        self.params = default_parameters
        self.run_params = run_parameters

    def sweep(self, parameter, values):
        new_param_set = self.params.copy()
        results = []
        for val in tqdm(values):

            try:
                new_param_set[parameter] = val
                row = ocean_row(new_param_set)                        
                result = row.run_ocean_row(*self.run_params)
...
class ocean_row:
    ...
    def plt_overview(self):
        ...
        pdom = self.create_domain() # create allowed domain to row
        ...
        fig,ax = plt_utils.plt_base_simple(xlims,ylims,10,10) # create figure
    ...
    def coords_to_polar(Xa,datets,hc,xlims=[],ylims=[],zlims=[]):
        ...
        *zlims: list [2]
        ...
        # add polar plot
        plt_utils.plt_modified_imshow(X,Y,Z,cmap='jet',interpolation='bilinear',vmin=zlims[0],vmax=zlims[1])
